...
use generic_array::typenum::{U20, U7};
...
impl<C, R> Matrix<C, R> {
    pub fn new<E>(cols: C, rows: R, scan_period: &u32) -> Result<Self, E>
    where
        ...
        R::Len: heapless::ArrayLength<bool>,
    {
        ...
        for (i, c) in self.cols.into_iter().enumerate() {
            ...
            for (j, r) in rows.into_iter().enumerate() {
                state_matrix.poll_update(j, i, r.is_high().ok().unwrap());
            }
            c.set_low().ok();
        }
...
impl Col {
    pub fn new(bounce_limit: Milliseconds, held_limit: Milliseconds, idle_limit: Milliseconds, scan_period: &u32) -> Col {
        Col {
            col: [KeyState::new(bounce_limit.integer(), held_limit.integer(), idle_limit.integer(), scan_period); 20]
        }
    }
}
...
impl RowArray {
    pub fn new(bounce_limit: Milliseconds, held_limit: Milliseconds, idle_limit: Milliseconds, scan_period: &u32) -> RowArray {
        RowArray {
            row: [Col::new(bounce_limit, held_limit, idle_limit, scan_period); 7]
        }
    }
}

/*
pub struct RowArrayIntoIterator {
    rowarray: RowArray,
    index: usize,
}

impl Iterator for RowArrayIntoIterator {
    type Item = RowArray;
    fn next(&mut self) -> Option<RowArray> {
        let result = self.rowarray[self.index];
        self.index += 1;
        Some(result)
    }
}
...
impl StateMatrix {
    ...
    /*pub fn new(bounce_limit: Milliseconds, held_limit: Milliseconds, idle_limit: Milliseconds, scan_period: Microseconds) -> StateMatrix {
        StateMatrix { // Create a two dimensional array of key states with a debounce delay of 5ms, a hold time of 5ms, and an idle limit of 500ms
            keys: [[KeyState::new(bounce_limit.integer(), held_limit.integer(), idle_limit.integer(), scan_period.integer()); 7]; 20],
        }
    }*/
    pub fn new(bounce_limit: Milliseconds, held_limit: Milliseconds, idle_limit: Milliseconds, scan_period: &u32) -> StateMatrix {
            let keys = RowArray::new(bounce_limit, held_limit, idle_limit, scan_period);
            StateMatrix {keys: keys}
    }
    ...
    pub fn poll_update(&mut self, r: usize, c: usize, high: bool) -> bool {
        let _change = KeyState::poll_update(&mut self.keys.row[r].col[c], high);
        false
    }

    // Get the individual state of a specific key
    pub fn get_state(&self, r: usize, c: usize) -> State {
        KeyState::get_state(&self.keys.row[r].col[c])
    }
